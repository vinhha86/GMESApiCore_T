...
public class Pcontract_PO_Runnable implements Runnable{
	...
	public Pcontract_PO_Runnable(PContract_PO po, IProductPairingService pairService, IPackingTypeRepository packing_repo,
			IPOrder_POLine_Service porder_line_Service, IPOrder_Product_SKU_Service porderskuService, IPOrderBOMSKU_Service porderBOMSKU_Service,
			ICutplanProcessingService cutplanProcessingService, IPOrderGrant_Service porderGrantService, IPOrderProcessing_Service pprocessRepository, IStockInService stockInService,
			IStockOutService stockOutService, List<PContractPO_Shipping> list_shipping) {
		this.po = po;
...
//		ship.setOrdercode(po.getOrdercode());
		...
		List<ProductPairing> p = pairService.getproduct_pairing_detail_bycontract(po.getOrgrootid_link(), po.getPcontractid_link(), po.getProductid_link());
		int total = 1;
		if(p.size()>0) {
			total = 0;
