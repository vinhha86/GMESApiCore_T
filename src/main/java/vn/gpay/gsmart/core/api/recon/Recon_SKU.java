...
import vn.gpay.gsmart.core.api.balance.Jitin_Stockout_OrderList_Response;
import vn.gpay.gsmart.core.api.balance.Jitin_Stockout_order_d_Data;
...
public class Recon_SKU implements Runnable{
	...
	private SKURecon_Data mat_sku;

	String token;
	CountDownLatch latch;
	
	Recon_SKU(
			List<SKURecon_Data> ls_skubalance,
			Long pcontractid_link,
			Long stockid_link,
			Long pcontract_poid_link,
			Long porderid_link,
			SKURecon_Data mat_sku,
			String token,
			CountDownLatch latch){
...
//			cal_invoice();
			...
			//Số đã xuất kho thành phẩm
			
			//Số đã huỷ theo biên bản huỷ thành phẩm
...
//            System.out.println(result);
	...
	//Tinh SL nguyen phu lieu da xuat kho
	private void cal_stockout_bycontract(){
		try {
            ...
            String result = restTemplate.postForObject(urlPost, request, String.class);
//            System.out.println(result);
            Jitin_StockoutList_Response ls_stockoutd = objectMapper.readValue(result, Jitin_StockoutList_Response.class);
...
//            		System.out.println(this.mat_sku.getMat_skuid_link() + "-" + stockoutD.getTotalmet_check());
	...
	private void cal_stockout_order_bycontract(){
		try {
            ...
            String result = restTemplate.postForObject(urlPost, request, String.class);
//            System.out.println(result);
            Jitin_Stockout_OrderList_Response ls_stockout_orderd = objectMapper.readValue(result, Jitin_Stockout_OrderList_Response.class);
